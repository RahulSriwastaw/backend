...
export async function generateImage(prompt, options = {}) {
  ...
  try {
    ...
    // Upload generated image to Cloudinary (Generated Images account)
...
/**
 * Generate with Google Gemini (Imagen)
 */
async function generateWithGoogleGemini(config, prompt, negativePrompt, quality, aspectRatio) {
  ...
  // Google Gemini Imagen API endpoint
...
async function generateWithCustomAPI(config, prompt, negativePrompt, quality, aspectRatio) {
  ...
  // Build request body based on custom settings
  const requestBody = {
    prompt,
    ...(negativePrompt && { negative_prompt: negativePrompt }),
    quality,
